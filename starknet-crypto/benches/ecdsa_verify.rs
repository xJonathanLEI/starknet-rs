...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let stark_key = Felt::from_bytes_be(&stark_key).unwrap();
    let msg_hash = Felt::from_bytes_be(&msg_hash).unwrap();
    let r_bytes = Felt::from_bytes_be(&r_bytes).unwrap();
    let s_bytes = Felt::from_bytes_be(&s_bytes).unwrap();
